...
app = Flask(__name__, template_folder=os.path.join(
    ROOT_DIR, '..', 'templates'), static_folder=os.path.join(
        ROOT_DIR, '..', 'static'))
app.secret_key = 'your_secret_key'

# Load the trained models, TF-IDF vectorizers, and label encoders
sentiment_model = joblib.load(os.path.join(
    ROOT_DIR,  'hack_model.pkl'))
sentiment_tfidf = joblib.load(os.path.join(
    ROOT_DIR,  'hack_tfidf_vectorizer.pkl'))
cyberbullying_model = joblib.load(os.path.join(
    ROOT_DIR,  'hack2_model.pkl'))
cyberbullying_tfidf = joblib.load(os.path.join(
    ROOT_DIR,  'hack2_tfidf_vectorizer.pkl'))
label_encoder = joblib.load(os.path.join(
    ROOT_DIR,  'label_encoder.pkl'))
    ...
    if request.method == 'POST':
        ...
        if file:
            try:
                ...
                # Check if the dataset is for sentiment analysis
                # or cyberbullying detection
                if 'reviews.text' in new_df.columns and 'reviews.rating'\
                        in new_df.columns:
                    # Sentiment Analysis using hack_model 
                    new_df = new_df.dropna(
                        subset=['reviews.text', 'reviews.rating'])
                    new_df['sentiment'] = new_df[
                        'reviews.rating'].apply(
                    ...
                    prediction_counts = pd.Series(
                        new_predictions).value_counts().to_dict()
                    
                    # Create a circular bar chart
                    fig, ax = plt.subplots(
                        figsize=(8, 8), subplot_kw=dict(aspect="equal"))
                    wedges, texts, autotexts = ax.pie(
                        prediction_counts.values(), wedgeprops=dict(width=0.5),
                        startangle=-40, autopct='%1.1f%%', textprops=dict(
                            color="w"))
                    
                    bbox_props = dict(
                        boxstyle="square,pad=0.3", fc="w", ec="k", lw=0.72)
                    kw = dict(
                        xycoords='data', textcoords='data', arrowprops=dict(
                            arrowstyle="-"))
                    for i, p in enumerate(wedges):
                        ang = (p.theta2 - p.theta1)/2. + p.theta1
                        y = np.sin(np.deg2rad(ang))
                        x = np.cos(np.deg2rad(ang))
                        horizontalalignment = {-1: "right", 1: "left"}[int(
                            np.sign(x))]
                        connectionstyle = f"angle,angleA=0,angleB={ang}"
                        kw["arrowprops"].update(
                            {"connectionstyle": connectionstyle})
                        ax.annotate(list(
                            prediction_counts.keys())[i], xy=(x, y), xytext=(
                    ...
                    new_predictions_decoded = label_encoder.inverse_transform(
                        new_predictions)
                    prediction_counts = pd.Series(
                        new_predictions_decoded).value_counts().to_dict()
...
def history():
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        cursor.execute(
            'SELECT action, timestamp FROM history WHERE user_id = %s', (
                session['user_id'],))
